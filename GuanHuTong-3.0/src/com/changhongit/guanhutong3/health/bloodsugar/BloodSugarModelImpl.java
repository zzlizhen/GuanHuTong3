...
public class BloodSugarModelImpl implements BloodSugarModel{
	
	private final int SUCCESS = 0;
	private final int FAILURE = 1;
	OnLoadedListener listener;
	
	public BloodSugarModelImpl(OnLoadedListener listener){
	...
	public void LoadData() {
		...
		new Thread() {
            public void run() {
            	String result = HttpUtil.getinstance().RequestBloodSugar(GhtApplication.mCurrentTerminal.getImei(), null, null, 1, 1);
	...
	Handler handler = new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			case SUCCESS:
				...
				try {
					list = XMLPullUtil.getinstance().parseBloodSugar(result);
