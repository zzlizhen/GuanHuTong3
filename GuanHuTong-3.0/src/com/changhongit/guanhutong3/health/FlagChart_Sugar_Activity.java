...
public class FlagChart_Sugar_Activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mList = new ArrayList<BloodSugarInfo>();
		type = getIntent().getExtras().getInt("type");
		sugar = "http://new.guanhutong.com.cn:9080/ilove.webapi/bloodsugars?imei="
	...
	public void setmData(){
		...
		for(int i = 0;i<mList.size();i++){
			mData[i] = Double.valueOf(mList.get(i).getFPG());
		}
	}
	...
	public void initView() {
		...
		if (mData != null && mData.length > 0) {
			...
			if (type != 3) {
				mFlagView= (GraphicalView)new Chart().executeSugar(this, tmp,
						mTimes,mData);
				layout.addView(mFlagView);
			} else {
				double[][] datas = { mData, mData_1 };

				View view = new Chart().execute1(this, tmp,
						datas);
				layout.addView(view);
			}
		}
	...
	public void parOxygenList(String str) {
		mList.clear();
		if (str != null) {
			BloodSugarParser parser = new BloodSugarParser();
			try {
				mList.addAll(parser
						.parseBloodSugarInfoList(new ByteArrayInputStream(
								str.getBytes())));
